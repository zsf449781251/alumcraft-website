"""Serve the AlumCraft website and deliver inquiry forms through SMTP.

The SMTP password is read from the environment at request time. It must never be
placed in the repository or sent to the browser.
"""
...
GLOBAL_RATE_LIMIT_MAX_REQUESTS = 60
...
PUBLIC_ROOT_FILES = {"robots.txt", "sitemap.xml"}
...
class MailSettings:
    ...
    def from_environment(cls) -> "MailSettings":
        to_email = os.getenv("INQUIRY_TO_EMAIL", "znegshifan@yushiglobal.cn").strip()
        username = os.getenv("SMTP_USERNAME", to_email).strip()
        settings = cls(
            host=os.getenv("SMTP_HOST", "smtp.exmail.qq.com").strip(),
    ...
    def validate(self) -> None:
        ...
        if not self.from_name or len(self.from_name) > 100 or "\r" in self.from_name or "\n" in self.from_name:
            raise MailConfigurationError("SMTP_FROM_NAME is invalid")


class RateLimiter:
    """Small in-memory limiter suitable for a single website process."""
    ...
    def allow(self, key: str, now: float | None = None) -> bool:
        ...
        with self._lock:
            ...
            # Bound memory even when attackers rotate spoofed or proxy-provided
            # client addresses. New addresses share a conservative overflow bucket.
...
class DeliveryTracker:
    """Remember in-flight and successful submissions to make browser retries safe."""
...
GLOBAL_RATE_LIMITER = RateLimiter(GLOBAL_RATE_LIMIT_MAX_REQUESTS, GLOBAL_RATE_LIMIT_WINDOW_SECONDS, max_clients=1)
...
def _valid_email(value: str) -> bool:
    return len(value) <= 254 and "\r" not in value and "\n" not in value and bool(EMAIL_PATTERN.fullmatch(value))
...
def parse_inquiry(data: Mapping[str, object]) -> Inquiry:
    ...
    message = _text(data, "message", 5000, required=True)
    if len(name) < 2 or len(message) < 10:
        raise InquiryError("input_too_short", "Please add a little more detail to your inquiry.")
    ...
    return Inquiry(
        ...
        email=email,
        country=_text(data, "country", 100),
        product_interest=_text(data, "product_interest", 150),
        quantity=_text(data, "quantity", 100),
        message=message,
        language=language,
...
def send_email(message: EmailMessage, settings: MailSettings) -> None:
    ...
    if settings.security == "ssl":
        with smtplib.SMTP_SSL(settings.host, settings.port, timeout=settings.timeout, context=context) as smtp:
...
class AlumCraftRequestHandler(SimpleHTTPRequestHandler):
    server_version = "AlumCraftWeb/1.0"
    ...
    def do_POST(self) -> None:  # noqa: N802 - stdlib handler API
        ...
        try:
            ...
            # Return a normal-looking response for honeypot submissions so bots
            # do not learn which field triggered the rejection.
            ...
            self._json_response(
                ...
                {"ok": False, "code": "mail_unavailable", "message": "Email delivery is temporarily unavailable."},
            )
        except (smtplib.SMTPException, OSError, TimeoutError) as exc:
            LOGGER.error("Inquiry email delivery failed (%s)", type(exc).__name__)
            self._json_response(
                HTTPStatus.BAD_GATEWAY,
                {"ok": False, "code": "delivery_failed", "message": "We could not deliver the inquiry right now."},
            )
        except Exception as exc:  # Keep unexpected implementation details out of the response.
    ...
    def _check_same_origin(self) -> None:
        ...
        if not origin_host or origin_host not in allowed_hosts:
            raise InquiryError("origin_not_allowed", "This request origin is not allowed.", HTTPStatus.FORBIDDEN)
    ...
    def _read_form_data(self) -> dict[str, object]:
        ...
        if content_length > MAX_BODY_BYTES:
            raise InquiryError("request_too_large", "The inquiry is too large.", HTTPStatus.REQUEST_ENTITY_TOO_LARGE)
        ...
        try:
            ...
            if content_type == "application/x-www-form-urlencoded":
                parsed = parse_qs(raw_body.decode("utf-8"), keep_blank_values=True, max_num_fields=20)
    ...
    def _is_public_static_path(self) -> bool:
        ...
        cleaned_parts = tuple(part for part in parts if part != "/")
        ...
        if len(cleaned_parts) == 1:
            ...
            return filename.endswith(".html") or filename in PUBLIC_ROOT_FILES or filename in {"chatbot", "pl", "ro"}
        ...
        return bool(allowed_extensions and remainder and PurePosixPath(remainder[-1]).suffix.lower() in allowed_extensions)
